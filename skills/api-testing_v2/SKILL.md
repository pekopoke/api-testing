---
name: api-testing
description: 根据源码或 API 文档（OpenAPI/Swagger/Postman）生成 REST 与 GraphQL 接口的集成测试，
...
- 为 REST 接口生成测试（可从源码或从 API 文档出发）
- **从 API 文档生成测试**：支持 OpenAPI 2.0/3.x（Swagger）、Postman Collection — 先用本目录下 `scripts/` 中的脚本得到端点清单
...
当接口由**文档**定义（而非仅源码）时，可使用以下格式，并用本 skill 目录下 `scripts/` 中的脚本得到端点清单：
...
**方式 A — 从 API 文档（OpenAPI / Swagger / Postman）：**
...
# 整理：端点与 HTTP 方法、路径/查询/请求体参数、请求/响应结构、鉴权与权限要求
```
...
- **结构**：按端点分 class（如 `TestGetUsers`、`TestCreateUser`），每个 class 内按场景写 `test_xxx`；使用 Arrange-Act-Assert；正向（成功码）与反向（400/401/403/404/409 等）都要写。
...
- 400（参数/校验错误）、401（未认证）、403（无权限）、404（未找到）、409（冲突）、422（语义错误）、500（服务错误）
...
**输入：** 接口路由/端点，或 API 文档文件（OpenAPI/Swagger/Postman）。  
